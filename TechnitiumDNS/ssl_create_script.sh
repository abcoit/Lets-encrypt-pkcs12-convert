...
# Write the .sh file
/bin/cat <<EOM >/etc/letsencrypt/renewal-hooks/deploy/sslconvert.sh
#!/bin/sh
...
sudo chmod +x /etc/letsencrypt/renewal-hooks/deploy/sslconvert.sh

# Run the script manually for the first time to generate the initial .pfx file
sudo /etc/letsencrypt/renewal-hooks/deploy/sslconvert.sh
